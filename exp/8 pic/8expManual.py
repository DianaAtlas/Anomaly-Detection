...
def manPvalue8exp(signal, amp1, bg):
    ...
    exp = bg * np.exp(-indexv / 100000000000000)
    ...
    from5 = int(mu5 - 1*sigma_training)
    to5 = int(mu5 + 2*sigma_training)

    from6 = int(mu6 - 1*sigma_training)
    to6 = int(mu6 + 2*sigma_training)

    from7 = int(mu7 - 1*sigma_training)
    to7 = int(mu7 + 2*sigma_training)

    from8 = int(mu8 - 1*sigma_training)
    to8 = int(mu8 + 2*sigma_training)
    ...
    # manual_01 = (np.trapz(aanorm[:, from8:to8], axis=1))
    #     #
    #     # manual_00 = (np.trapz(bbnorm[:, from8:to8], axis=1) )
...
#
# plt.title('P-Value VS Signal Magnitude Exponential BG')
# plt.ylabel('P-Value')
# plt.yscale('log')
# plt.xlabel('bachground Magnitude [Events/GeV]')
# plt.plot(bg1222, pvalue2, label='DAta driven')
# plt.legend()
# plt.grid()
# plt.show()
#
...
#                    index=['bg', 'pvalue'])
# df1.to_excel("8expManual_BG111111.xlsx")
...
for i in range(10):
     ...
     bg1222.append(amp)
     pvalue2.append(manPvalue8exp(5, amp, 200))
     amp = amp + 10
...
df1 = pd.DataFrame([bg1222, pvalue2],
                   index=['amp', 'sigma5'])
df1.to_excel("FULLMANUALEXP.xlsx")
