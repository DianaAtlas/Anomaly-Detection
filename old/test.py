...
def gaussian_generator(mu, sigma, amp, bground):
    result = []
    result = 0.5*(amp*(stats.norm.pdf(indexv, mu, sigma)) + stats.norm.pdf(indexv+1, mu, sigma)) + 2*bground
    return result
...
x = np.linspace(mu_training - 10 * sigma_training, mu_training + 10 * sigma_training, 100)
bground_training = np.zeros(precision) + const_training


print('bg if vector', np.array(bground_training).shape,bground_training)

si_bg = bground_training + stats.norm.pdf(x, mu_training, sigma_training)

gaussian_training = gaussian_generator(mu_training, sigma_training, amp_training, bground_training)
print(np.array(gaussian_training).shape, gaussian_training)
data_combined = []
label_combined = []

for _ in range(2000):
    a, b = fluctuation_manufacture(gaussian_training, bground_training)
...
data_combined_norm = np.array(data_combined) / 1000



# print('length pf data', len(data_combined), '\n', len(a), '\n', a)
# print(data_combined, '\n', label_combined)
# print('normalized data', data_combined_norm)
...
x_test = np.linspace(mu_training - 10 * sigma_training, mu_training + 10 * sigma_training, 100)
...
data_testing_00 = []
label_testing = []
gaussian_test = gaussian_generator(mu_test, sigma_test, amp_testing, bground_test)


from1 = int(mu_test - 1.5*sigma_test)
to = int(mu_test + 1.5*sigma_test)
...
for _ in range(numofexampls):
    aa, bb = fluctuation_manufacture(gaussian_test, bground_test)
...
model.add(Dense(5, batch_size=50,
                ...
                activity_regularizer=regularizers.l1(0.0001)))    # Kernel Regularization regulates the weights
...
#-----------------------------------------
# ----- Trying 2 layer model--------------
#-----------------------------------------
# model = Sequential()
# model.add(Dense(100, batch_size=50, activation='relu'))
# model.add(Dropout(0.4))
# model.add(Dense(50,batch_size=50, activation='relu'))
# model.add(Dropout(0.4))
# model.add(Dense(1,batch_size=50, activation='sigmoid'))
# model.compile(loss='binary_crossentropy',
#               optimizer='rmsprop',
#               metrics=['accuracy'])
# history = model.fit(np.array(data_combined_norm), np.array(label_combined), epochs=1000)
# print(model.summary())
# print('NN predicted', model.predict([np.array(data_testing_norm)]),'true value', '\n', lable_testing)
...
#         numofevents_1.append(0.5 * (amp * (stats.norm.pdf(i, mu, sigma) + stats.norm.pdf(i + 1, mu, sigma)) + 2 * bground))
