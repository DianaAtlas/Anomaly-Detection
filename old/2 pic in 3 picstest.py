...
def gaussian_generator2(mu1, mu2,sigma, amp, bground):
    numofevents_1 = []
    for i in range(precision):
        numofevents_1.append((0.5 *(amp * (stats.norm.pdf(i, mu1, sigma) + stats.norm.pdf(i + 1, mu1, sigma)))
                             +0.5 *(amp * (stats.norm.pdf(i, mu2, sigma) + stats.norm.pdf(i + 1, mu2, sigma)) + 2 * bground)))
...
data_testing_00 = []
label_testing = []
gaussian_test = gaussian_generator2(mu1_test,mu2_test, sigma_test, amp_testing, bground_test)
